...
BOOST_AUTO_TEST_SUITE(heap_)

BOOST_AUTO_TEST_CASE(empty)
{
    heap_sort(empty_list, 0);
}

BOOST_AUTO_TEST_CASE(normal)
{
    heap_sort(normal_list, NORMAL_SIZE);
...
{
    heap_sort(normal_list, NORMAL_SIZE, false);
    for (int i = 0; i < NORMAL_SIZE; ++i) {
...
BOOST_AUTO_TEST_CASE(random)
{
    ...
    heap_sort(random_list, RANDOM_SIZE);
