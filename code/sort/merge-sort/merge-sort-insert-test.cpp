...
BOOST_AUTO_TEST_SUITE(merge_insert)

BOOST_AUTO_TEST_CASE(empty)
{
    merge_sort_insert(empty_list, 0, 0);
}

BOOST_AUTO_TEST_CASE(normal)
{
    merge_sort_insert(normal_list, 0, NORMAL_SIZE);
...
{
    merge_sort_insert(normal_list, 0, NORMAL_SIZE, false);
    for (int i = 0; i < NORMAL_SIZE; ++i) {
...
BOOST_AUTO_TEST_CASE(random)
{
    ...
    merge_sort_insert(random_list, 0, RANDOM_SIZE);
